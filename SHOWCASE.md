...
The full, copy-pasteable sequence — including every shape, color and `\n`
line-break — is the deck section of [`examples/tour.sh`](examples/tour.sh) and
...
![dashboard — KPI band, 8-region table with live totals, and a native bar chart](assets/showcase/dashboard.png)
...
> banded table, the live `=SUM`/`=AVERAGE` totals row, the native **FY Revenue by
> Region** bar chart with data labels, and the data-bar / color-scale conditional
> formatting are all written straight into the `.xlsx`. This screenshot is page 1
> of the workbook as LibreOffice prints it — a second **Quarterly Revenue Trend**
> line chart follows on the next page.
...
aioffice render   dashboard.xlsx --to png --scope /Dashboard/B5:I18 -o dashboard.png
```
...
One script regenerates all three artifacts and renders every PNG, using only
`aioffice` on your PATH (and Chrome for the `render --to png` steps):
...
下面的每一个像素，都由一个 AI 原生 CLI 生成。没有 Microsoft Office、没有模板、
没有任何手工修饰——三个真实的 Office 文件（`.pptx` / `.xlsx` / `.docx`），由
[`aioffice`](README.zh-CN.md) 一条命令一条命令地构建，再由同一个二进制渲染成
PNG。每张卡片展示渲染结果、它是什么、用了多少条命令，以及——折叠在下方的——
**逐字** 命令序列。用一个脚本即可亲手重建整个作品集：
[`examples/tour.sh`](examples/tour.sh)。

> 这里的 PNG 正是 `aioffice render --to png` 的输出（一次无头 Chrome 截图）。
> 源文件（`assets/showcase/deck.pptx`、`dashboard.xlsx`、`report.docx`）是合法
> 的 OOXML，可在真正的 PowerPoint、Excel、Word 中打开。
...
完整、可直接复制的命令序列（含每个图形、颜色与 `\n` 换行）就是
[`examples/tour.sh`](examples/tour.sh) 的 deck 部分，以及
[`assets/showcase/deck.commands.txt`](assets/showcase/deck.commands.txt)。
