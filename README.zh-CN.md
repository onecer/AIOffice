...
macOS 上直接下载的二进制首次运行可能被 Gatekeeper 拦截，执行一次 `xattr -d com.apple.quarantine <路径>`（脚本与 Homebrew 已为你处理；二进制尚未签名公证——见 [docs/SIGNING.md](docs/SIGNING.md)）。安装后 `aioffice version` 应输出 `{"ok":true,"data":{"name":"aioffice","version":"1.6.0",…}}`。
...
- **2125 个测试**横跨 7 个项目（Core 124 · Word 612 · Excel 572 · Pptx 675 · MCP 87 · Preview 24 · Render 31），每次提交全绿。
