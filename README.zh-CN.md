...
**给你的 agent 一个 Office 引擎。** AIOffice 让 agent 像调用函数一样创建、查询、编辑、渲染、预览、校验真实的 `.docx` / `.xlsx` / `.pptx`：**一条命令进，恰好一个 JSON 信封出**。100% 自研，纯 C#/.NET——**一个约 36 MB 的单文件二进制，无需安装 Office，零运行时依赖，不包装任何第三方引擎。**

```bash
# 安装（任选其一）——随后用 `aioffice version` 验证
npx aioffice version                       # 免安装，适合 CI / MCP 宿主
npm install -g aioffice                     # 全局安装 `aioffice` 到 PATH
...
aioffice mcp    # 同一个文档引擎，以 MCP 工具形式走 stdio
```

### 为什么选 AIOffice

- **图文文档入口** —— 从 [docs/README.zh-CN.md](docs/README.zh-CN.md) 开始读。
- **100% 自研单文件二进制** —— 纯 C#/.NET，直接无损读写 OOXML（DocumentFormat.OpenXml + ClosedXML）。无 Office、无云端、不包装引擎、无运行时需要安装。
- **三种真实格式，一个工具** —— 创建并编辑可在真正的 Word、Excel、PowerPoint 中打开的 `.docx`、`.xlsx`、`.pptx`。
- **会教学的错误** —— 每次失败都返回一个带可执行 `suggestion`（路径错误时还带 `candidates`）的 JSON 信封，让 agent 自我纠正而非猜测。
- **render → look → fix** —— 通过系统浏览器把任意节点渲染为 PNG，*看到*你做出的东西；走 MCP 时图片直接内联返回。
- **冻结的 1.0 契约** —— 一个稳定、机器可读的能力面（`surfaceVersion` 1.0），agent 可放心依赖。见 [CONTRACT.md](CONTRACT.md)。
- **CLI = MCP，一套心智模型** —— 当前源码树暴露 **19 个 CLI 动词**与 **19 个 MCP 工具**。学一次，既可在 shell 里跑，也可走 stdio。

## 眼见为实

三个真实的 Office 文件——一套路演幻灯片、一份营收看板、一份能力报告——**全部由 `aioffice` 一条命令一条命令地构建**。构建时没装 Office，没有模板，没有手工修饰。下面的截图，是把这些文件原样用 **LibreOffice** 打开的样子——由第三方 Office 渲染器独立证明 `aioffice` 写出的是真实、合法、任何 Office 应用都能忠实渲染的 OOXML。完整作品集（含每张图背后的逐字命令序列）见 **[SHOWCASE.md](SHOWCASE.md)**；用一个脚本即可复现全部三个产物：**[`examples/tour.sh`](examples/tour.sh)**。

| | | |
|---|---|---|
| [![deck.pptx —— 带原生图表的深色路演幻灯片](assets/showcase/deck-1.png)](SHOWCASE.md#1--产品路演幻灯片--deckpptx) | [![dashboard.xlsx —— 区域营收看板](assets/showcase/dashboard.png)](SHOWCASE.md#2--区域营收看板--dashboardxlsx) | [![report.docx —— 排版整洁的能力报告](assets/showcase/report.png)](SHOWCASE.md#3--能力报告--reportdocx) |
| **`deck.pptx`** —— 6 页深色幻灯片 + 一个原生柱形图（17 条命令） | **`dashboard.xlsx`** —— KPI 带、实时 `=SUM`/`=XLOOKUP`、2 个图表（11 条命令） | **`report.docx`** —— 表格公式、LaTeX→Office Math、引文（18 条命令） |

<details>
<summary>它们是怎么做出来的——以及在一个更小例子上的 render → look → fix 闭环</summary>

上面三个产物由 **[`examples/tour.sh`](examples/tour.sh)** 构建（每条命令逐字可见），并在 **[SHOWCASE.md](SHOWCASE.md)** 中有文档说明。下面是同一工作流的一个自包含微缩版——三个小文件加上 render → look → fix 闭环——你可以直接粘进一个空目录：
...
这个微缩版用的是与完整作品集相同的闭环：`create` → 批量 `edit` → `validate` → `render --to png` → 查看 → `edit` 修正。xlsx 的 PNG 展示的是 HTML 渲染器画出的内容：单元格、格式与公式缓存结果；柱状图真实存在于文件中（见 `get '/Sheet1/chart[1]'`），用 Excel 打开即可看到。三个精修产物及其逐字命令见 **[SHOWCASE.md](SHOWCASE.md)** 与 **[`examples/tour.sh`](examples/tour.sh)**。
...
| **CLI = MCP 同一套心智模型** | 当前 main 暴露 19 个 CLI 动词和 19 个 MCP 工具，学一次，shell 和 stdio 两种接入 |
...
macOS 上直接下载的二进制首次运行可能被 Gatekeeper 拦截，执行一次 `xattr -d com.apple.quarantine <路径>`（脚本与 Homebrew 已为你处理；二进制尚未签名公证——见 [docs/SIGNING.md](docs/SIGNING.md)）。安装后 `aioffice version` 应输出带当前安装版本的 JSON 信封。

### 在你的 agent 里用它（MCP）

AIOffice 把**同一个文档引擎暴露两次**——一个 CLI 和一个 stdio **MCP server**。运行 `aioffice mcp --workspace <目录>`，把 agent 指过去：
...
```bash
aioffice mcp     # stdio MCP 服务器 —— 同一文档引擎上的 19 个工具
```
...
`preview_open` / `preview_selection`（实时预览 + 人工点选回读）已于 M1（v0.2.0）注册；`office_audit`（无障碍 + 质量 lint）是第 15 个工具，M7（v0.8.0）加入；`office_diff`（语义对比）是第 16 个工具，M8（v0.9.0）加入；`office_convert`（跨格式互转）是第 17 个工具，M9（v0.10.0）加入。当前 main 还新增 `preview_mark` 与 `preview_goto`，MCP 工具数到 19，同时文档契约仍保持 `surfaceVersion: "1.0"`。全部工具 schema 的 token 预算上限 3,500，并由测试强制。
...
| `preview <open\|selection\|close\|mark\|unmark\|marks\|goto> <file> [path] [--port N]` | localhost 实时预览；人点选 -> `selection` 返回规范路径；marks/goto 支持审阅标注 |
| `doctor` | 环境 / 运行时 / 处理器诊断 |
...
| `mcp` | stdio MCP 服务器 |
| `plugin <install\|uninstall\|list\|status>` | CLI-only 宿主安装器，写入 Claude、Codex、opencode、TonoBraid 配置 |
| `version` | 版本信息 |
...
```
                 ┌─────────────────────────────────────────────┐
   agent/human → │  src/AIOffice.Cli   （aioffice，19 动词）    │
   MCP client  → │  src/AIOffice.Mcp   （stdio，19 工具）       │
                 ├─────────────────────────────────────────────┤
...
**上手：** [docs/README.zh-CN.md](docs/README.zh-CN.md) —— 图文文档总览 · [docs/INSTALL.md](docs/INSTALL.md) —— 四种安装方式 · [docs/MCP-SETUP.md](docs/MCP-SETUP.md) —— 接入 Claude / Cursor / 任意 MCP 宿主 · [SKILL.md](SKILL.md) —— AI 上手指南 · [docs/COOKBOOK.md](docs/COOKBOOK.md) —— 10 个复制即用配方 · [docs/SIGNING.md](docs/SIGNING.md) —— 代码签名 / 公证路线图
...
[MIT](LICENSE)。见 [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)——全部依赖均为宽松许可（MIT：DocumentFormat.OpenXml、ClosedXML、ModelContextProtocol C# SDK；BSD-2-Clause：Markdig），无捆绑第三方二进制。
