...
| `help [topic]` | addressing · selectors · properties-docx/xlsx/pptx · errors · **equations**（docx+pptx）· **embeds** · **rtl** · **sections** · **audit** · **diff** · **convert** |
| `mcp` | stdio MCP 服务器 |
| `version` | 版本信息 |
...
- **契约冻结** —— 稳定的 v1.0 命令面记录于 **[CONTRACT.md](CONTRACT.md)**，并通过 `schema` 与 `doctor` capabilities 中的 `surfaceVersion`（`1.0-rc`）声明：信封结构、冻结的错误码集合、寻址语法、退出码、op/view/工具词表。
...
最后一个功能里程碑已交付；1.0 API 稳定性承诺之前剩下的是**稳定化，而非新功能**：

- **最终回归扫描** —— 对冻结的契约面（信封、错误码、寻址、op/view/工具词表）做一次完整的跨平台回归，确认无漂移。
- **API 冻结签收** —— 在 [CONTRACT.md](CONTRACT.md) 契约签收后，把 `surfaceVersion` 从 `1.0-rc` 升为 `1.0`，纳入版本化兼容承诺。
- **文档打磨** —— 对内置 help 主题、DESIGN/MCP/PARITY 文档与契约做最后一遍。
- **（1.0 后候选）插件机制** —— 运行时发现的外部格式 handler，第三方无需 fork 即可加格式。
