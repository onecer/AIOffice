...
CLI 动词与 MCP 工具 **1:1 镜像**（§2.7），一套心智模型。共 13 个能力。
...
### 2.7 MCP 工具面（13 个，stdio，官方 C# SDK）
...
| `preview_open` | —（M1） | v0 返回 `unsupported_feature`，suggestion 指向 `office_render` |
...
### M1 — Render 完整 + Preview

- `render --to html`（docx/xlsx）与 `--to svg`（pptx 每页）做到布局可信；`--to png` 通过无头浏览器探测实现（探测不到 → unsupported_feature + 安装建议）。
- watch 模式 + 浏览器实时预览 + 选区读取（`preview_*` MCP 工具转正）。
- pptx master/layout 寻址。
