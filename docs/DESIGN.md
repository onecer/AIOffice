...
CLI 动词与 MCP 工具 **1:1 镜像**（§2.7），一套心智模型。v0 起 13 个能力，M1 加 `preview`（14），M7 加 `audit`（15），M8 加 `diff`（16），M9 加 `convert`（17）。M10 的嵌入对象（`embed`/`extract`）与 pptx 公式搭载在既有 `office_edit`/`office_read`/`office_get` 上，工具数维持 **17**。M10 起 `schema` 与 `doctor` capabilities 声明稳定的 **`surfaceVersion`**——面向 AI 的契约版本，独立于工具包版本（见 [CONTRACT.md](../CONTRACT.md)）；1.0.0 起其值为 **`1.0`**（M10 期间为 `1.0-rc`）。1.1.0、1.2.0、1.3.0、1.4.0 与 1.5.0（五个 1.0 之后功能版本）均**纯增量**：1.1 扩图表/iconSet/引用/媒体/效果/切换；1.2 加 pptx SmartArt 创建/连接符/分组（`group`·`ungroup` 是 `add` type 值而非新 op kind）、docx 图表目录/索引/邮件合并、xlsx 表单控件/轻量保护/numberFormat 命名预设；1.3 加 xlsx+pptx 图表润色（`dataLabels`/`legend`/`axisTitles`/`trendline`/`errorBars`/`gridlines`/`secondaryAxis`）、xlsx 高级条件格式（`formula`/`topBottom`/`aboveBelowAverage`）+ 透视计算字段（`calculatedFields`）、docx 正文形状/文本框（`add type:shape`/`textBox`）+ 传统表单域（`add type:formField`）+ 主题编辑（`set /theme`）、pptx 3D 模型（`add type:model3d` + `model3d_as_media` 警告）+ 动作路径动画（`motionPath`）；1.4 **填平动态数组缺口**——xlsx `=FILTER`/`UNIQUE`/`SORT`/`SORTBY`/`SEQUENCE`/`RANDARRAY`/`TRANSPOSE` 写入时求值并溢出（`get` 报 `spillRange`、占用区 `spill_blocked`）+ `RATE`/`IRR`/`XIRR`/`NPV`/`PV`/`FV`/`PMT`/`NPER` 财务函数求值缓存（以上不再触发 `formula_not_evaluated`）+ 模拟运算数据表（`add type:dataTable`）、docx 邮件合并执行（`template` 的 `--data` 接受记录数组 + `--output` 模式，`office_template` 增可选 `output` 参）+ IF 域（`add type:ifField`）+ 页面边框（`set /section[i] {pageBorder}`）、pptx 缩放导航（`add type:zoom`）+ 点击触发动画（`triggerOn:"@N"`）+ 内建表格样式（`add type:table {style}`）；1.5 **填平现代标量函数缺口**——xlsx `=XLOOKUP`/`IFS`/`SWITCH`/`LET`/`MAXIFS`/`MINIFS`/`AVERAGEIFS` 写入时求值缓存（`=TEXTSPLIT` 溢出，以上不再触发 `formula_not_evaluated`）+ 方案管理器（`add type:scenario` + `set {applyScenario}`，已重算）+ 单变量求解（`set {goalSeek}`，`goal_seek_no_solution` 警告）、docx 表格单元格公式（`set {formula}` → 缓存 `w:fldSimple`，`table_formula_cached` 警告）+ 构建基块（`add type:buildingBlock`/`buildingBlockRef`）+ 节行号（`set /section[i] {lineNumbers}`）、pptx 嵌入字体（`add /fonts {type:font}` + `embedAll`）+ 动作按钮（`add type:actionButton`）+ 自定义版式（`add type:layout {placeholders}` + 幻灯片 `layoutName`）——全部搭载既有 `office_edit`/`office_read`/`office_get`/`office_template`/`office_help`，工具数维持 **17**、`surfaceVersion` 维持 **`1.0`**，仅包版本递增到 **1.5.0**。1.6.0 是**分发与上手版本**（npm 包 / Homebrew formula / 一行安装脚本 / SKILL + cookbook 文档），二进制能力零变化，仍 **17 工具 / `surfaceVersion` `1.0`**。1.7.0（第六个 1.0 之后功能版本，纯增量）**补齐打印就绪、相机工具、计算模式与公式深度**：xlsx 打印完整性（`printTitleRows`/`printTitleCols`/`pageBreaks`/`fitToPage`/居中/网格/`printHeader`/`printFooter`）+ 链接图片/相机工具（`add type:linkedPicture`，`linked_picture_static` 警告，`/SheetN/linkedPicture[i]`）+ 计算设置（`set /` `calculationMode`/`iterativeCalc`/…，`get /` 回 `{calculation, workbookProtection}`）、docx 首字下沉（`set {dropCap}`）+ 图片水印（`add type:watermark {image}`）+ STYLEREF/SYMBOL/QUOTE 域 + 带编号显示公式（`number:…`，`/equation[@num=…]`，`equation_numbers_cached` 警告）、共享 LaTeX→OMML 转换器深化（`\begin{aligned|gathered|cases}`/`\binom`/`\overbrace`/`\underbrace`/`\iint`/`\iiint`/更多重音关系——向后兼容、pptx 自动获得）、pptx 备注/讲义母版（`set /notesMaster`/`/handoutMaster`）+ 动画时间（`repeat`/`rewind`/`autoReverse`）+ 表格单元格对齐（`valign`/边距/`textDirection`）——全部搭载既有工具，工具数维持 **17**、`surfaceVersion` 维持 **`1.0`**，新 `office_help` 主题 `print-setup`/`masters`，仅包版本递增到 **1.7.0**。
