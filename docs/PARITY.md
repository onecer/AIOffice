...
> 本账本只记录**能力**（capability），不映射任何命令语法。OfficeCLI（参考源码 `/tmp/office_research/OfficeCLI`，`SKILL.md` + `src/officecli/Handlers/{Word,Excel,Pptx}` + `schemas/help/`）的能力清单是长期对齐的北极星；但 AIOffice 是 100% 自研（C#/.NET，DocumentFormat.OpenXml + ClosedXML），命令与参数**刻意不**与上游 1:1 还原 —— 我们的对外表面是自己的 15 动词 AI 原生命令面（见 `docs/DESIGN.md`；M1 起含 `preview`）。
>
...
| 🔜 M1 | M1 计划但尚未落地的剩余项（顺延至 M8 窗口） |
| 📋 M2 | M2 计划但尚未落地的剩余项（顺延至 M8 窗口） |
| 🗺 M8 | 深水区余项：pptx/xlsx 公式（复用 `AIOffice.Word.Equations` 转换器）、语义 diff 动词、跨格式转换 docx↔pptx、OLE 对象、插件机制、现代 xlsx 批注打磨、动画预设++、多效果动画链/motion path、dump 回放 |
...
| CSV/TSV 导入 xlsx | `create orders.xlsx --from orders.csv`：RFC 4180（引号/嵌入逗号换行）、分隔符嗅探（`, ; tab \|`）或 `delimiter` 强制、`--title` 命名 sheet；单元格类型化（数字/日期/布尔转换，`007` 类前导零码**保持文本**）；>50k 单元格走 SAX 流式写 | ✅ M5 已实现 | 自研解析（ExcelCsv.cs）+ ExcelHandler.Csv.cs（CreateFrom 钩子，命令层路由 + 沙箱）；表头识别（AutoFilter + 冻结首行）→ 📋 余项 |
| markdown 桥（docx） | `create report.docx --from notes.md`：Markdig GFM 导入（标题→样式、粗斜删/行内代码 run、嵌套列表、管道表格、链接/图片/引用/代码块/分隔线，raw HTML 与缺图降级为 warning）；`read --view markdown` 把 body 导出回 GFM，结构与导入 round-trip | ✅ M5 已实现 | Markdig（BSD-2-Clause）AST 走查（WordHandler.Markdown.cs / WordHandler.MarkdownExport.cs）；IFormatHandler `CreateFrom` 默认实现 = `unsupported_feature`（pptx 等无导入器格式诚实拒绝） |
...
| 域（field） | `add type:field`：pageNumber（PAGE）/ numPages（NUMPAGES）/ date（DATE + format picture）/ docTitle（TITLE），`leadingText` 前缀字面文本 —— 「Page X of Y」页脚 = 两个域组合；body 与页眉页脚段落均可；get/render 回报 kind + instruction | ✅ M5 已实现 | `SimpleField`（WordHandler.Fields.cs）；REF/SEQ 等其余 24 种上游域 + fieldchar/instrtext 低层 → 📋 余项 |
...
| RTL | sheetView rightToLeft、cell/comment direction | 🗺 M8 | OpenXml `SheetView.RightToLeft`（M6 RTL 只覆盖 docx；xlsx sheetView RTL 顺延 M8） |
...
| 图片 | 插入（PNG/JPEG）/位置/尺寸（单位换算，缺省守纵横比）/name；返回稳定 `shape[@id=N]` 路径；M7 起 **alt 文本/标题**（`set {altText}`/`{altTitle}`，写 `p:nvPicPr/cNvPr/@descr`/`@title`，无障碍审计读取，SVG 渲染出 `<title>`） | ✅ M2 已实现（M7 alt 文本） | `ImagePart` + `P.Picture`（PptxImages.cs）；src 必经沙箱；SVG/裁剪/滤镜 → 🗺 M8 |
...
| 公式（equation） | OMML 公式形状 | 🗺 M8 | `a14:m` 内嵌 Math（M6 公式已落地 docx；pptx 公式形状顺延 M8，复用 `AIOffice.Word.Equations` 转换器） |
...
| RTL | shape/notes direction、图表轴 RTL | 🗺 M8 | `a:bodyPr@rtlCol` / `a:pPr@rtl`（M6 RTL 只覆盖 docx；pptx 形状 RTL 顺延 M8） |
...
| 范围 | ✅ 已实现（M0–M6） | 🔜 M1 余项 | 📋 M2 余项 | 🗺 深水区（M7） | ❌ 不计划 | 合计 |
|---|---|---|---|---|---|---|
| 跨格式通用 | 26 | 3 | 5 | 4 | 3 | 41 |
| docx | 22 | 1 | 6 | 5 | 0 | 34 |
| xlsx | 25 | 4 | 6 | 6 | 0 | 41 |
| pptx | 20 | 4 | 5 | 10 | 0 | 39 |
| **合计** | **93** | **12** | **22** | **25** | **3** | **155** |
