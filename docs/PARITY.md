...
| 条件格式 | cellIs（7 算子含 between）/ colorScale（2-3 色）/ dataBar / containsText / iconSet（1.1，3/4/5 图标集，`set`/`reverse`/`showValue`）五类，`/Sheet1/conditionalFormat[i]` 寻址 | ✅ M2 已实现（1.1 + iconSet） | `IXLConditionalFormat`（ExcelConditionalFormats.cs）；formulacf / topN / aboveAverage / dateoccurring / duplicate / unique 六类 → 🗺 M3 |
...
| 图表 | bar/line/pie + **scatter/area**（M3）+ **doughnut/radar/bubble/stackedBar/percentStackedBar/stackedArea/combo**（1.1）增删读（`/Sheet1/chart[1]` 寻址、anchor 简写）；scatter 双数值轴、首列为数值 X | ✅ M1 已实现（M3 + scatter/area；1.1 + 7 类） | **ClosedXML 不支持图表**：自研 OpenXml `ChartPart` + `DrawingsPart`（`ExcelCharts.cs`）；轴/系列深编辑 → 📋 余项 |
