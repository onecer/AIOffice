...
```sh
VERSION=v1.6.0 AIOFFICE_BIN=/usr/local/bin \
...
outward-facing command. Run these yourself after tagging `v1.6.0`.

### 1. Cut the v1.6.0 release

Tag `v1.6.0` and upload the 6 binaries + `SHA256SUMS` (your existing release
flow). The install scripts default to the GitHub "latest" release, so they pick
up v1.6.0 automatically once it is published.

### 2. Homebrew tap

1. Create a public repo `onecer/homebrew-tap`.
2. Copy `dist/Formula/aioffice.rb` into it as `Formula/aioffice.rb`.
3. Fill the four `sha256` values (each marked `TODO(human)`) from the v1.6.0
   `SHA256SUMS`:
   ...
   them. The formula already has `version "1.6.0"`.
