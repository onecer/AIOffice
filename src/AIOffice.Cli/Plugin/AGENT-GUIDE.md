...
5. **Charts**: `roundRect` card (`{panel}`/white) → chart on top → `move <card> position:"back"`. Chart carries `dataLabels:{show:"value",position:"outEnd"}, legend:"bottom", axisTitles:{category,value}, gridlines:{major:true,minor:false}, seriesColors:["{accent1}","{accent2}"]`.
