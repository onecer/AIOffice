    ...
    [
        ...
        new("edit",
            ...
            [
                ...
                new("type", "<element>", "Element type for --add, e.g. p, table, slide, shape, image, comment, reply, style, pivot, conditionalFormat, toc, watermark, endnote, sectionBreak, animation, note, row, col, field, dataValidation, sparkline, caption (docx), crossRef (docx), slicer (xlsx), source/citation/bibliography (docx), media (pptx audio/video). v1.2: smartart/connector/group/ungroup (pptx), tableOfFigures/indexEntry/index/mergeField (docx), formControl (xlsx). v1.4: dataTable (xlsx what-if), ifField (docx «IF» merge field), zoom (pptx slide/section/summary navigation). v1.5: scenario (xlsx), buildingBlock/buildingBlockRef (docx AutoText), font (pptx embedded .ttf), actionButton (pptx). v1.7: linkedPicture (xlsx camera-tool snapshot of a range). A dynamic-array formula on a cell (=FILTER/UNIQUE/SORT/SORTBY/SEQUENCE/RANDARRAY/TRANSPOSE) spills automatically on --set value=…; see 'aioffice help formulas'. v1.7 set-prop additions: xlsx print titles/pageBreak/fitToPage/printHeader/printFooter/center + calculationMode/iterativeCalc; docx dropCap + watermark image + STYLEREF/SYMBOL/QUOTE fields + equation number; pptx /notesMaster & /handoutMaster set-paths + animation repeat/rewind/autoReverse + table-cell valign/margins. Deeper LaTeX→OMML (cases/aligned/binom/braces/multi-integral/numbering); see 'aioffice help equations', 'help print-setup', 'help masters'."),
