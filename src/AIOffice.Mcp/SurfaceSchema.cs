...
public static class SurfaceSchema
{
    ...
    [
        ...
        Verb("edit", "Apply an atomic batch of set/add/remove/move/replace/accept/reject/extract ops: all-or-nothing single save, auto-snapshot, optional rev guard. add types include embed (any file as an OLE object), equation (LaTeX→OMML, docx+pptx), dataTable (xlsx what-if), ifField (docx «IF» merge field), zoom (pptx slide/section/summary nav), scenario (xlsx changing-cell what-if set), buildingBlock/buildingBlockRef (docx reusable AutoText), font (pptx embedded .ttf) and actionButton (pptx); set props gain goalSeek + applyScenario (xlsx), formula on table cells + lineNumbers on section (docx) and embedAll (pptx font). A dynamic-array formula (=FILTER/UNIQUE/SORT/…) on a cell spills automatically, and scalar XLOOKUP/IFS/SWITCH/LET/TEXTJOIN now evaluate to cached values; extract writes an embedded object's bytes back out.", "office_edit",
            [("file", "document path"),
