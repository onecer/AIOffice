...
internal static class PptxProperties
{
    ...
    /// <summary>The typed value behind one custom property (vt:lpwstr/r8/bool/filetime).</summary>
    ...
    /// vt:* entries (string/number/bool/date) in the CustomFilePropertiesPart.
    /// </summary>
    public static string Set(PresentationDocument doc, JsonObject props)
    {
        ...
        if (setCore.Count == 0 && setCustom.Count == 0)
        {
            throw new AiofficeException(
                ...
                "Example: {\"op\":\"set\",\"path\":\"/properties\",\"props\":{\"title\":\"Q3 Deck\",\"custom\":{\"Reviewed\":true}}}.");
        }
    ...
    private static void SetCustomProperty(PresentationDocument doc, string name, JsonNode? node)
    {
        ...
        var properties = part.Properties ??= new Properties();

        // Remove any existing property of the same name (case-insensitive, as PowerPoint treats them).
    ...
    private static OpenXmlElement CustomVariant(JsonNode node)
    {
        if (node is JsonValue value)
        {
            ...
            // Numbers (int/long/double/decimal, parsed or in-memory) become a real numeric variant.
            if (Units.TryNumber(value, out var d) && !value.TryGetValue<string>(out _))
            {
    ...
    private static void RenumberCustomProperties(Properties properties)
    {
        var pid = 1;
    ...
    private static string NodeToString(JsonNode node) =>
        node is JsonValue v && v.TryGetValue<string>(out var text) ? text : node.ToJsonString();
}
