...
| File             | Role                                                                 |
| ---------------- | -------------------------------------------------------------------- |
| `package.json`   | name `aioffice`, version `1.27.2`, `bin`, `postinstall`, `files`.     |
...
`package.json` is at `1.27.2`. On install, `install.js` downloads release
`v{package.version}` = **`v1.27.2`** by default. That tag does not exist yet — the
...
Because `v1.27.2` is not tagged yet, and because the `onecer/AIOffice` repo is
...
When the real `v1.27.2` tag exists, the **same flow runs with no env overrides**:
`install.js` downloads `https://github.com/onecer/AIOffice/releases/download/v1.27.2/<asset>`
...
# 0. Make sure the v1.27.2 GitHub release exists with all 6 binaries + SHA256SUMS.
